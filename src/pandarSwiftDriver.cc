...
bool PandarSwiftDriver::poll(void) {
  ...
  bool skipSleep = true;
  uint32_t pointcloudStartTime = GetTickCount();
  uint32_t pointcloudEndTime = GetTickCount();
  uint32_t faultmessageStartTime = GetTickCount();
  uint32_t faultmessageEndTime = GetTickCount();
  for (int i = 0; i < m_iPandarScanArraySize; ++i) {
    faultmessageEndTime = pointcloudEndTime = GetTickCount();
    if ((pointcloudEndTime - pointcloudStartTime) > m_pPandarSwiftSDK->m_configMap["without_pointcloud_udp_warning_time"]) {
      printf("can not receive pointcloud udp data for %d ms\n", m_pPandarSwiftSDK->m_configMap["without_pointcloud_udp_warning_time"]);
      pointcloudStartTime = pointcloudEndTime;
    }
    if ((faultmessageEndTime - faultmessageStartTime) > m_pPandarSwiftSDK->m_configMap["without_faultmessage_udp_warning_time"]) {
      printf("can not receive faultmessage udp data for %d ms\n", m_pPandarSwiftSDK->m_configMap["without_faultmessage_udp_warning_time"]);
      faultmessageStartTime = faultmessageEndTime;
    }
    ...
    if (rc == GPS_PACKET) {
      // gps packet;
      PandarGPS packet;
      if (parseGPS(&packet, &m_arrPandarPackets[m_iPktPushIndex][i].data[0],
                   GPS_PACKET_SIZE) == 0) {
    ...
    if (rc == FAULT_MESSAGE_PACKET) {
      faultmessageStartTime = faultmessageEndTime;
      m_pPandarSwiftSDK->processFaultMessage(
          m_arrPandarPackets[m_iPktPushIndex][i]);
      i--;
      continue;
    }
  ...
  m_iPktPopIndex = temp;
  if (m_bNeedPublish == false) m_bNeedPublish = true;
  // else
  // 	printf("CPU not fast enough, data not published yet, new data
  // comming!!!\n");
  return true;
}
